...
var _ = Describe("VirtualMachineTemplate", Ordered, func() {
	It("should process a VirtualMachineTemplate and return a VirtualMachine", func() {
		...
		processed, err := tplClient.TemplateV1alpha1().VirtualMachineTemplates(NamespaceTest).Process(context.Background(), template.Name, opts)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create a VirtualMachine from VirtualMachineTemplate", func() {
		...
		opts := subresourcesv1alpha1.ProcessOptions{}
		processed, err := tplClient.TemplateV1alpha1().VirtualMachineTemplates(NamespaceTest).CreateVirtualMachine(
			context.Background(),
			template.Name,
			opts)
		Expect(err).NotTo(HaveOccurred())
