...
The user creating a `VirtualMachineTemplateRequest` must have `create` permission
on the `virtualmachinetemplaterequests/source` subresource in the namespace of
the source `VirtualMachine`. This is enforced by a `ValidatingAdmissionPolicy`
and ensures that permission to use a VM as a source is granted deliberately.
