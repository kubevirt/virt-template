...
func (r *VirtualMachineTemplateRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	...
	defer func() {
		...
		if patchErr := helper.Patch(ctx, tplReq); patchErr != nil {
	...
	if err := r.setOwnerReferences(ctx, tplReq, tpl); err != nil {
...
func (r *VirtualMachineTemplateRequestReconciler) processRequest(
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(snap), snap); client.IgnoreNotFound(err) != nil {
	...
	} else if snap.Labels[v1alpha1.LabelRequestUID] != string(tplReq.UID) {
		setProgressingCondition(ctx, tplReq, metav1.ConditionFalse, v1alpha1.ReasonFailed)
...
func (r *VirtualMachineTemplateRequestReconciler) setOwnerReferences(
	ctx context.Context, tplReq *v1alpha1.VirtualMachineTemplateRequest,
...
func validateRequest(tplReq *v1alpha1.VirtualMachineTemplateRequest) error {
	if tplReq.Spec.VirtualMachineRef == nil {
		return errors.New("virtualMachineRef cannot be nil")
	}
...
func getDvName(tplReq *v1alpha1.VirtualMachineTemplateRequest, volumeName string) string {
	return apimachinery.GetStableName(tplReq.Name, string(tplReq.UID), volumeName)
}
...
func transformVolume(ctx context.Context, volumes *[]virtv1.Volume, volName string) string {
	for i, vol := range *volumes {
		...
		dvName := volName
		if (*volumes)[i].DataVolume != nil {
			dvName = (*volumes)[i].DataVolume.Name
		}

		(*volumes)[i] = virtv1.Volume{
			Name: volName,
