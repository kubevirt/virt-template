...
func usage() string {
  ...
  {{ProgramName}} process mytemplate

  # Process a template by name from the cluster.
  {{ProgramName}} process --name mytemplate

  # Process a template from a local file.
  {{ProgramName}} process --file mytemplate.yaml

  # Process a template from stdin.
  {{ProgramName}} process --file -

  # Process a template in a specific namespace.
  {{ProgramName}} process --name mytemplate --namespace production

  # Process a template with parameters.
  {{ProgramName}} process --name mytemplate --param key1=value1 --param key2=value2

  # Process a template with parameters from a file.
  {{ProgramName}} process --name mytemplate --params-file params.yaml

  # Process a template and output as JSON.
  {{ProgramName}} process --name mytemplate --output json

  # Process a local template file with parameters.
  {{ProgramName}} process --file mytemplate.yaml --param diskSize=50Gi --param memory=4Gi
  ...
  {{ProgramName}} process --file mytemplate.yaml --print-params

  # Combine local file processing with parameters file and JSON output.
  {{ProgramName}} process -f mytemplate.yaml --params-file params.json -o json

  # Process template from current directory and redirect output to file.
  {{ProgramName}} process --file mytemplate.yaml > vm.yaml

  # Process template with multiple parameters and save as JSON.
  {{ProgramName}} process mytemplate -p cpu=4 -p memory=8Gi -p diskSize=100Gi -o json > vm.json

  # Process a template from different namespace with parameters.
  {{ProgramName}} process mytemplate -n dev -p diskSize=20Gi

  # Process template and directly apply it to the cluster.
  {{ProgramName}} process mytemplate -n staging | kubectl apply -f -
  `
}
