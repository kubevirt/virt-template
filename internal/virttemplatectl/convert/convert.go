...
func usage() string {
  ...
  {{ProgramName}} convert mytemplate

  # Convert a template by name from the cluster.
  {{ProgramName}} convert --name mytemplate

  # Convert a template from a local file.
  {{ProgramName}} convert --file mytemplate.yaml

  # Convert a template from stdin.
  {{ProgramName}} convert --file -

  # Convert a template in a specific namespace.
  {{ProgramName}} convert --name mytemplate --namespace production

  # Convert a template and output as JSON.
  {{ProgramName}} convert --name mytemplate --output json

  # Convert a local template file.
  {{ProgramName}} convert --file mytemplate.yaml

  # Convert template from current directory and redirect output to file.
  {{ProgramName}} convert --file mytemplate.yaml > vmt.yaml

  # Convert template and save as JSON.
  {{ProgramName}} convert mytemplate -o json > vmt.json

  # Convert a template from different namespace.
  {{ProgramName}} convert mytemplate -n dev

  # Convert template and directly apply it to the cluster.
  {{ProgramName}} convert mytemplate -n staging | kubectl apply -f -
  `
}
